...
public class GroupController extends BaseController {
    ...
    public Message saveUsers(@PathVariable("users") String usersIds,
        ...
        Group persistGroup = groupService.find(groupid);
        persistGroup.getUsers().clear();
        if (!usersIds.equals("clear")) {
            String[] uids = usersIds.split(","); // TODO:要处理users为空字串的情况，没有办法解除全部的用户
            for (String uid : uids) {
                User persistUser = userService.find(String.valueOf(uid));
    ...
    public Message saveRoles(@PathVariable("roles") String rolesIds,
        ...
        if (!rolesIds.equals("clear")) {
            ...
            for (String rid : rids) {
                Role persistRole = roleService.find(String.valueOf(rid));
